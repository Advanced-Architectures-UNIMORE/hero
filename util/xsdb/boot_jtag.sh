...
hero_petalinux_dir=${hero_root_dir}/petalinux/zcu102_90MHz # HERO Petalinux Folder
...
${hero_root_dir}/util/xsdb/reset_jtag.sh -i ${hw_server_ip} -p ${hw_server_port}
...
if [ -z "${hero_bitstream}" ]; then
    ...
    petalinux-boot --jtag --u-boot --fpga --bitstream ${hero_bitstream} -v --hw_server-url ${hw_server_ip}:${hw_server_port}
fi
