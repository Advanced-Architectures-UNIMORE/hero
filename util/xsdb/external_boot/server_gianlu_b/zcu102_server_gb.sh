...
# if [ -n "$NO_IIS" ]; then
#   petalinux-config --silentconfig --get-hw-description="$HERO_ROOT/hardware/fpga/hero_exil$TARGET"
# else
...
if [ -f "$LOCAL_CFG" ] && grep -q BR2_HERO_ETH_IP_ADDR "$LOCAL_CFG"; then
  echo '# CONFIG_SUBSYSTEM_ETHERNET_PSU_ETHERNET_3_USE_DHCP is not set' >> project-spec/configs/config
  sed -e 's/BR2_HERO_ETH_IP_ADDR/CONFIG_SUBSYSTEM_ETHERNET_PSU_ETHERNET_3_IP_ADDRESS/;t;d' "$LOCAL_CFG" >> project-spec/configs/config
  sed -e 's/BR2_HERO_ETH_NETMASK/CONFIG_SUBSYSTEM_ETHERNET_PSU_ETHERNET_3_IP_NETMASK/;t;d' "$LOCAL_CFG" >> project-spec/configs/config
  sed -e 's/BR2_HERO_ETH_GATEWAY/CONFIG_SUBSYSTEM_ETHERNET_PSU_ETHERNET_3_IP_GATEWAY/;t;d' "$LOCAL_CFG" >> project-spec/configs/config
else
