...
#ifndef __HAL_RISCV_RISCV_MULTI_CL_H__
#define __HAL_RISCV_RISCV_MULTI_CL_H__
...
static inline unsigned int riscv_core_id() {
  int hart_id;
...
static inline unsigned int riscv_cluster_id() {  int hart_id;
#if RISCV_VERSION >= 4 && !defined(RISCV_1_7)
#if PULP_CHIP == CHIP_GAP
  asm("csrr %0, 0x014" : "=r" (hart_id) : );
#else
